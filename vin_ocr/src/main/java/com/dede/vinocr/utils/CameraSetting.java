...
public class CameraSetting {
    ...
    public static CameraSetting getInstance(Context context) {
        if (single == null) {
            single = new CameraSetting(context.getApplicationContext());
        }
        return single;
    }
...
//				parameters.setExposureCompensation((int)(parameters.getMinExposureCompensation()*0.7));
                ...
                Toast.makeText(
                        context, "该设备不支持闪光灯", Toast.LENGTH_SHORT)
                        .show();
            }
        } catch (Exception e) {
            e.printStackTrace();
...
//		 }
        ...
        //	parameters.setExposureCompensation(0);
//		parameters.setExposureCompensation((int)(parameters.getMaxExposureCompensation()*0.7));
        ...
        if (isSetZoom) {
            if (parameters.isZoomSupported()) {
                parameters.setZoom((int) (parameters.getMaxZoom()));  //因为近距离时有时不好对焦，现将焦距拉远，以便近距离时能清晰对焦   可自定义值
            }
        }
